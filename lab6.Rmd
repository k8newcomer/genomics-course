...
ggplot(daily_report, aes(x = Long, y = Lat, size = Confirmed/1000)) +
    ...
    theme_bw() +
    geom_point(shape = 10, color='purple', fill='purple', alpha = 0.5) +
...
ggplot(daily_report, aes(x = Long, y = Lat, trans="log", size = Confirmed*10)) +
    borders("state", colour = "white", fill = "grey90") +
    geom_point(aes(x=Long, y=Lat, size=Confirmed, color=Confirmed),stroke=F, alpha=1) +
    scale_size_continuous(name="Cases", range=c(1,4),trans="log", 
...
us <- map_data("state")
nv_us <- subset(us, region == "california")
counties <- map_data("county")
nv_county <- subset(counties, region == "california")
state_join <- left_join(nv_county, daily_report, by = c("subregion" = "Admin2")) 
# plot state map
